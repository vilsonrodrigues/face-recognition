...
if __name__ == '__main__':
    ...
    model_path_mob_facenet = os.getenv('MODEL_PATH_MOB_FACENET',
                            default='models/mobilefacenet_prep.onnx')

    model_path_ultra_light = os.getenv('MODEL_PATH_ULTRA_LIGHT',
                            default='models/mobilefacenet_prep.onnx')

    # scale configs

    num_cpus_to_basic_func = float(os.getenv('NUM_CPUS_TO_BASIC_FUNC',
                            default='0.3'))

    num_cpus_to_models = float(os.getenv('NUM_CPUS_TO_MODELS',
                            default='1.0'))

    num_gpus_to_models = float(os.getenv('NUM_GPUS_TO_MODELS',
                            default='0.0'))

    num_actors_to_basic_func = int(os.getenv('NUM_ACTORS_TO_BASIC_FUNC',
                                        default='1'))

    num_actors_to_models = int(os.getenv('NUM_ACTORS_TO_MODELS',
                                        default='1'))
    ...
    ds_payloads = (ds
        ...
        .map_batches(
            ...
            fn_kwargs=dict(
                input_key='foto',
                output_key='foto_base64')
        )
    )

    ds_embeddings = (ds_payloads
        .map_batches(
        ...
        .map_batches(
            ...
            zero_copy_batch=True,
            fn_constructor_kwargs=dict(
                    model_path=model_path_mob_facenet,
                    input_key='face',
                    backend=backend
            )
        )
        .map_batches(
            ...
            batch_size=batch_size_funcs,
            zero_copy_batch=True,
            fn_kwargs=dict(input_key='face',
                            output_key='face_base64')
        )
    ).materialize()

    ds_payloads = ds_payloads.drop_columns(['image','foto'])

    payloads = (ds_payloads
                .zip(ds_embeddings.select_columns(['face_base64']))
                .to_pandas()
                .to_dict('records'))
